...
import { advanceRoad, createRoad, stages, grabRoller, moveRoller, releaseRoller, roadPose, resumeRoad } from './domain/road.ts';
...
if (dev) {
  try {
    const data = import.meta.hot?.data.snapshot ?? JSON.parse(sessionStorage.getItem(snapshotKey) ?? 'null');
    if (data?.key === snapshotKey) {
      ...
      if (['excavator', 'dump-truck', 'roller', 'traffic', 'complete'].includes(candidate.phase)
        ...
        tuning = { ...settings, dragTilt: defaultTuning.dragTilt };
        state = resumeRoad(candidate, tuning);
        if (stages.includes(data.targetStage)) targetStage = data.targetStage;
      }
    }
...
function save() {
  if (!dev) return;
  const snapshot = { key: snapshotKey, state, tuning, targetStage };
  if (import.meta.hot) import.meta.hot.data.snapshot = snapshot;
  try { sessionStorage.setItem(snapshotKey, JSON.stringify(snapshot)); } catch { /* Optional storage. */ }
}
function ready() {
...
function connect(scene: ReturnType<typeof createScene>, audio: ReturnType<typeof createAudio>) {
  const events = new AbortController();
  const options = { signal: events.signal };
  const canvas = scene.canvas;
  let active: { id: number; phase: MissionPhase; startY: number; offset: Point } | undefined;
  ...
  function release() {
    setState({ ...state, excavator: releaseBucket(state.excavator), truck: truckInput(state.truck, { type: 'cancel' }, tuning), roller: releaseRoller(state.roller) });
  }
  ...
  canvas.addEventListener('pointerdown', event => {
    ...
    active = { id: event.pointerId, phase: state.phase, startY: event.clientY, offset: state.phase === 'excavator' ? { x: state.excavator.bucket.x - point.x, y: 0, z: state.excavator.bucket.z - point.z } : { x: state.roller.x - point.x, y: 0, z: 0 } };
  ...
  canvas.addEventListener('pointermove', event => {
    if (!active || active.id !== event.pointerId || active.phase !== state.phase) return;
    if (state.phase === 'dump-truck') {
      setState({ ...state, truck: truckInput(state.truck, { type: 'drag', upwardPx: active.startY - event.clientY }, tuning) });
    } else {
      const point = scene.onPlane(event.clientX, event.clientY, state.phase === 'roller' ? 1.2 : HOME.y);
      if (!point) return;
      if (state.phase === 'excavator') setState({ ...state, excavator: moveBucket(state.excavator, { x: point.x + active.offset.x, y: HOME.y, z: point.z + active.offset.z }) });
      if (state.phase === 'roller') setState({ ...state, roller: moveRoller(state.roller, point.x + active.offset.x) });
    }
  }, options);
  canvas.addEventListener('pointerup', event => {
    if (active?.id !== event.pointerId) return;
    active = undefined;
    setState({ ...state, excavator: releaseBucket(state.excavator), truck: truckInput(state.truck, { type: 'release' }, tuning), roller: releaseRoller(state.roller) });
    if (canvas.hasPointerCapture(event.pointerId)) canvas.releasePointerCapture(event.pointerId);
  }, options);
  for (const type of ['pointercancel', 'lostpointercapture'] as const) canvas.addEventListener(type, event => { if (active?.id === event.pointerId) cancelPointer(); }, options);
  window.addEventListener('blur', cancelPointer, options);
  document.addEventListener('visibilitychange', () => { if (document.hidden) { cancelPointer(); save(); } previousTime = performance.now(); }, options);
  ...
  function animate(now: number) {
    ...
    if (active && active.phase !== state.phase) cancelPointer();
...
const program = Effect.scoped(Effect.gen(function* () {
  const scene = yield* Effect.acquireRelease(Effect.try({ try: () => createScene(app.querySelector('.canvas-host')!), catch: cause => new Error('無法建立 3D 畫面', { cause }) }), value => Effect.sync(() => value.dispose()));
  const audio = yield* Effect.acquireRelease(Effect.sync(createAudio), value => Effect.promise(() => value.dispose()));
  yield* Effect.acquireRelease(Effect.sync(() => connect(scene, audio)), dispose => Effect.sync(dispose));
  yield* Effect.never;
}));
void Effect.runPromise(program, { signal: lifetime.signal }).catch((error: unknown) => {
  ...
  app.querySelector('.loading')?.remove(); app.querySelector('.world')!.append(message);
});
if (import.meta.hot) { import.meta.hot.accept(); import.meta.hot.dispose(() => { save(); lifetime.abort(); }); }
