# Town Crew — Web road-repair mission
...
- `npm run build` checks TypeScript and creates `web/dist/`.
...
The old `stage=ready` and `stage=dumping` truck bookmarks still work.
...
The product also includes firefighting, outdoor rescue, and other town missions.
They should own their own interaction rules; they do not have to use this
road-repair sequence.

- `src/domain/excavator.ts`: fixed-length IK, reach limits, rock clearing and
  automatic pickup/unload/return behavior, with plain numerical data.
- `src/domain/dump-truck.ts`: truck gesture rules and animation timing.
- `src/domain/road.ts`: the road mission, two roller passes, vehicle transitions,
  repaired surface, traffic timing, and snapshot recovery.
- `src/geometry.ts` and `src/vehicles.ts`: reusable shapes and vehicle visuals.
- `src/scene.ts`: environment, camera, picking, world-space pointer projection,
  feedback, and rendering the mission state.
- `src/audio.ts`: browser audio unlock, cues, mute, and disposal.
- `src/ui.ts`: game HUD and development controls.
- `src/main.ts`: browser input, runtime orchestration, development snapshots,
  and Effect's scoped ownership of the scene, audio, events, and animation loop.
...
Local Chrome browser validation covers the full mouse and emulated-touch flow,
background rejection, cancellation, partial roller movement, completion and
restart. Development reload/HMR is checked after the first roller pass, including
...
JavaScript chunk over 500 kB (approximately 155 kB gzipped).

## Technology comparison

The original truck slice returned from an actual source edit to an interactive
frame in **451 ms** in one warm local Chrome run. That result predates the full
mission and is not a Bevy/Web comparison or a guarantee for this version.

Compare identical parameter, gesture-rule, and visual edits in Bevy and Web,
including the time needed to return to the relevant stage. Record repeated warm
runs and cold starts separately. Aim to halve the complete iteration time
without regressing the real-device experience.

Web is a potential shipping implementation. A later Bevy port remains an option
with real implementation and validation costs. The existing Rust source and
project plan remain the Bevy reference; this experiment does not change them.
