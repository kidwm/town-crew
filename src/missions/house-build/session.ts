...
export function createHouseSession(app: HTMLDivElement, dev: boolean, onHome: () => void) {
  ...
  const stage: Stage = dev && stages.includes(query as Stage) ? query as Stage : 'gravel';
  const devKey = `town-crew:house:dev:v1:${location.search}`;
  let state = createHouse(stage);
  try { state = resumeHouse(dev ? JSON.parse(sessionStorage.getItem(devKey) ?? 'null') : loadProgress('house-build')) ?? state; } catch { /* Optional storage. */ }
  function save() {
  ...
  function connect(scene: ReturnType<typeof createHouseScene>, audio: ReturnType<typeof createAudio<keyof typeof houseSounds>>) {
    ...
    function animate(now: number) {
      ...
      app.querySelector<HTMLElement>('.roof-picker')!.hidden = state.phase !== 'decorate';
