...
import { createHouse, resumeHouse, grab, release, dragGravel, moveChute, drive, moveLoad, advance, chooseColor, startRoofLift, progress, stages, PARTS, ROOF_COLORS, isCrane, isDelivery } from './domain/house.ts';
...
const instructions: Record<Stage, string> = { gravel: '按住車斗前端，往上拉', concrete: '把出料口移到光圈，慢慢填滿三區', 'delivery-one': '按住車子，往右拖到停車位', 'crane-one': '把材料拖到房子上，吊車幫你放好', 'delivery-two': '按住車子，往右拖到停車位', 'crane-two': '把材料拖到房子上，吊車幫你放好', 'roof-color': '先挑屋頂顏色，再請吊車吊上去', decorate: '歡迎入住！', complete: '歡迎搬進新家！' };
  ...
  function connect(scene: ReturnType<typeof createHouseScene>, audio: ReturnType<typeof createAudio<keyof typeof houseSounds>>) {
    ...
    function animate(now: number) {
      ...
      label('.house-instruction', state.phase === 'crane-two' && state.action === 'leaving'
        ...
        : drop?.accepted ? '對準了！吊車幫你放好' : instructions[state.phase]);
