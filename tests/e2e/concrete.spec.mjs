...
test('concrete follows the foundation centres and aligns an off-centre grip', async ({ page }, info) => {
  ...
  await page.addInitScript(state => sessionStorage.setItem('town-crew:play:v1:house-build', JSON.stringify(state)), createHouse('concrete'));
  ...
  const project = (point, y) => {
    const v = new THREE.Vector3(point.x, y, point.z).project(camera);
  ...
  // The final region cannot fill while the first one is still empty.
  await down(project({ x: -0.6, z: 1.8 }, 1.1));
  await move(project(POUR_TARGETS[2], 0.185));
  await page.waitForTimeout(1300);
  await expect(app).toHaveAttribute('data-pours', '0');
  for (const [i, target] of POUR_TARGETS.entries()) {
    // Aim at the visible gravel surface, not a point suspended above it.
    await move(project(target, 0.185));
    await expect(app).toHaveAttribute('data-pours', String(i + 1));
    await up();
    const saved = await page.evaluate(() => JSON.parse(sessionStorage.getItem('town-crew:play:v1:house-build')));
    expect(saved.chute).toEqual(target);
    if (i < 2) {
      await page.screenshot({ path: info.outputPath(`filled-zone-${i + 1}.png`) });
